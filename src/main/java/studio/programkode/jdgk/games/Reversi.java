...
public class Reversi extends Plugin
{
    ...
    public void draw(API window) {
        window.drawTextCentered(
                this.center.x, 50,
                "%s".formatted(this.player.equals(Piece.BLACK)
                    ? "Player 1 (BLACK)"
                    : "Player 2 (WHITE)"
                )
        );
        ...
        window.drawTextCentered(
                this.center.x, 100,
                "Turn %s".formatted(this.turn)
        );

        window.drawTextCentered(
                this.center.x, window.getHeight() - 150,
                "BLACK: %s".formatted(this.pieces[0])
        );

        window.drawTextCentered(
                this.center.x, window.getHeight() - 125,
                "WHITE: %s".formatted(this.pieces[1])
        );

        if (this.gameFinished) {
            window.drawTextCentered(
                    this.center.x, window.getHeight() - 75,
                    "%s".formatted(this.pieces[0] == this.pieces[1]
                        ? "TIE"
                        : "Winner: %s".formatted(
        ...
        if (this.boardArea.contains(window.getCursor())) {
            int x = this.getXFromMouseX(window.getMouseX());
            int y = this.getYFromMouseY(window.getMouseY());

            if (this.board[x][y] == null) {
                window.fill(this.player == Piece.WHITE ? this.colors.get("white") : this.colors.get("black"));

                window.drawCircle(
                        this.upperLeftCorner.x + this.boardDelta*(x + 1) - 8,
                        this.upperLeftCorner.y + this.boardDelta*y + 9,
                        16,
                        Anchor.TOP_RIGHT
                );
